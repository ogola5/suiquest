...
const index_js_2 = require("../../../client/chain/grpc/index.js");
const index_js_3 = require("../../../client/chain/rest/index.js");
...
class MsgBroadcasterWithPk {
    ...
    constructor(options) {
        ...
        this.privateKey =
            options.privateKey instanceof index_js_1.PrivateKey
                ? options.privateKey
                : index_js_1.PrivateKey.fromHex(options.privateKey);
    ...
    async broadcast(transaction, accountDetails) {
        ...
        if (index_js_5.ofacWallets.includes(privateKey.toHex())) {
    ...
    async broadcastWithFeeDelegation(transaction) {
        ...
        if (txTimeoutOnFeeDelegation) {
            const latestBlock = await new index_js_2.ChainGrpcTendermintApi(endpoints.grpc).fetchLatestBlock();
            const latestHeight = latestBlock.header.height;
            timeoutHeight = new utils_1.BigNumberInBase(latestHeight)
                .plus(txTimeout)
                .toNumber();
        }
        const transactionApi = new index_js_4.IndexerGrpcWeb3GwApi(endpoints.web3gw || endpoints.indexer);
    ...
    async getAccountDetails(accountDetails) {
        ...
        if (useRest) {
            ...
            const accountDetailsResponse = await new index_js_3.ChainRestAuthApi(endpoints.grpc).fetchAccount(privateKey.toBech32());
        ...
        const { privateKey, endpoints } = this;
        const accountDetailsResponse = await new index_js_2.ChainGrpcAuthApi(endpoints.grpc).fetchAccount(privateKey.toBech32());
    ...
    async getTimeoutHeight() {
        ...
        if (useRest) {
            const latestBlock = await new index_js_3.ChainRestTendermintApi(endpoints.grpc).fetchLatestBlock();
            const latestHeight = latestBlock.header.height;
            return new utils_1.BigNumberInBase(latestHeight).plus(txTimeout);
        }
        const latestBlock = await new index_js_2.ChainGrpcTendermintApi(endpoints.grpc).fetchLatestBlock();
        const latestHeight = latestBlock.header.height;
        return new utils_1.BigNumberInBase(latestHeight).plus(txTimeout);
